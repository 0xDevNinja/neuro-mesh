...
pub mod pallet {
    ...
    pub enum Event<T: Config> {
        ...
        /// [subnet_id, owner]
        SubnetUpdated { subnet_id: u32, owner: T::AccountId },
        /// A subnet was retired
        /// [subnet_id, owner]
        SubnetRetired { subnet_id: u32, owner: T::AccountId },
    }
        ...
        pub fn update_subnet(
            ...
            Subnets::<T>::try_mutate(subnet_id, |maybe_subnet| {
                ...
                if let Some(schema) = input_schema {
                    subnet.input_schema =
                        schema.try_into().map_err(|_| Error::<T>::SchemaTooLarge)?;
                }

                if let Some(schema) = output_schema {
                    subnet.output_schema =
                        schema.try_into().map_err(|_| Error::<T>::SchemaTooLarge)?;
                }
...
mod tests {
    ...
    fn create_subnet_reserves_deposit() {
        new_test_ext().execute_with(|| {
            ...
            assert_eq!(initial_balance - final_balance, SubnetDeposit::get());
        });
    }

    #[test]
    fn create_subnet_with_max_emission_weight_succeeds() {
        new_test_ext().execute_with(|| {
            // Percent::from_percent saturates at 100%, so 101 becomes 100%
            // which is valid. Verify max weight (100%) is accepted.
            assert_ok!(SubnetRegistry::create_subnet(
            ...
            let subnet = SubnetRegistry::subnets(0).unwrap();
            assert_eq!(subnet.emission_weight, Percent::from_percent(100));
        });
    }
